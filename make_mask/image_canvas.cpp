...
void ImageCanvas::loadImage(const QString &filename) {
	if (!_image.isNull() )
		saveMask();
	base_name = filename;
	_img_file = dir +"/"+filename;
	_mask_file = dir + "/begin_mask/" + filename.left(filename.size()-4)+".png";
	_watershed_file = dir + "/mask/" + filename.left(filename.size() - 4) + ".png";
...
void ImageCanvas::saveMask() {
	...
	if (isFullZero(_mask.id))
		return;
	xy_imwrite(_mask_file, _mask.id);
	if (!_watershed.id.isNull()) {
		...
		xy_imwrite(_watershed_file, watershed);
	}
    _undo_list.clear();
    _undo_index = 0;
