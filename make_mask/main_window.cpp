	...
	: QMainWindow(parent, flags)
{
    ...
    clear_mask_action->setShortcut(Qt::Key_R);
    close_tab_action->setShortcut(Qt::CTRL + Qt::Key_W);
	undo_action->setEnabled(false);
	redo_action->setEnabled(false);
	//
	watersh_action = new QAction(tr("&runWatershed"), this);
    ...
    menuEdit->addAction(watersh_action);
    menuEdit->addAction(opendir_action);

	tabWidget->clear();
	connect(opendir_action, SIGNAL(triggered()), this, SLOT(on_actionOpenDir_triggered()));
	connect(button_watershed      , SIGNAL(released())                        , this, SLOT(runWatershed()  ));
    connect(watersh_action        ,SIGNAL(triggered())                       , this, SLOT(runWatershed()  ));
